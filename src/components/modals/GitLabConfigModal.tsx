...
function ConnForm({ conn, developers, onChange, onDelete, isOnly }: ConnFormProps) {
  ...
  function addDev(devId: string) {
    // Start blank. A developer can hold several GitLab accounts in Team, and only this
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 10, padding: '12px 14px', background: 'var(--surface2)', borderRadius: 8, border: '1px solid var(--border)' }}>
      ...
      <div>
        <span style={labelStyle}>
          ...
          <a href="https://gitlab.com/-/user_settings/personal_access_tokens" target="_blank" rel="noreferrer" style={{ display: 'inline-flex', alignItems: 'center', gap: 2, color: 'var(--accent)', textDecoration: 'none', marginLeft: 4 }}><Icon name="external" size={11} />create</a>
        </span>
        ...
        <div style={{ borderTop: '1px solid var(--border)', paddingTop: 10 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
            {developers.filter((d) => d.id in (conn.developerUsernames ?? {})).map((d) => {
              ...
              const available = identityList(d.gitlabUsername).filter((u) => !chosen.some((c) => c.toLowerCase() === u.toLowerCase()))
              return (
