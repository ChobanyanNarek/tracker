...
export async function fetchJiraBoardIssues(t: Transport, config: JiraConfig, boardId: number, assigneeEmail: string): Promise<JiraFetchResult> {
  const res = await t.post('/pm-tracker/jira-board-issues', {
  ...
  if (!res.ok) {
    ...
    throw new Error(`Jira ${res.status}: ${text.slice(0, 300) || 'request failed'}`)
  }
...
export async function fetchBoardProjectKeys(t: Transport, config: JiraConfig, boardId: number, _assigneeEmails?: string[]): Promise<string[]> {
  ...
  const keys = await fetchBoardIssueKeys(t, config, boardId).catch(() => [] as string[])
...
export async function fetchBoardIssueKeys(t: Transport, config: JiraConfig, boardId: number, _assigneeEmails?: string[]): Promise<string[]> {
  const res = await t.post('/pm-tracker/jira-board-keys', {
      baseUrl: config.baseUrl.trim(),
      email: config.email.trim(),
      ...authFor(config),
      boardId,
...
export async function fetchJiraBoards(t: Transport, config: JiraConfig): Promise<JiraBoardInfo[]> {
  const res = await t.post('/pm-tracker/jira-boards', {
      baseUrl: config.baseUrl.trim(),
      email: config.email.trim(),
      ...authFor(config),
    })
...
export async function fetchJiraSprints(t: Transport, config: JiraConfig, boardId: number): Promise<JiraSprintInfo[]> {
  const res = await t.post('/pm-tracker/jira-sprints', {
      baseUrl: config.baseUrl.trim(),
      email: config.email.trim(),
      ...authFor(config),
      boardId,
...
export async function fetchJiraTimeTracking(t: Transport, config: JiraConfig): Promise<number> {
  try {
    const res = await t.post('/pm-tracker/jira-time-tracking', {
        baseUrl: config.baseUrl.trim(),
        email: config.email.trim(),
        ...authFor(config),
      })
...
export async function fetchJiraStatuses(t: Transport, config: JiraConfig): Promise<JiraStatusInfo[]> {
  const res = await t.post('/pm-tracker/jira-statuses', {
      baseUrl: config.baseUrl.trim(),
      email: config.email.trim(),
      ...authFor(config),
    })
...
export async function fetchConnectionProjectKeys(t: Transport, config: JiraConfig): Promise<string[]> {
  try {
    const { issues } = await fetchJiraIssues(t, config, 'ORDER BY updated DESC')
...
export async function fetchJiraIssues(t: Transport, config: JiraConfig, jql: string): Promise<JiraFetchResult> {
  const res = await t.post('/pm-tracker/jira-search', {
      baseUrl: config.baseUrl.trim(),
      email: config.email.trim(),
      ...authFor(config),
      jql,
